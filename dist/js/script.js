window.addEventListener('DOMContentLoaded', function () {
    ...
    function tabs(elem) {
        ...
        const contents = document.querySelectorAll(`.row .${elem}__item`);
        ...
        for (let i = 0; i < tabs.length; i++) {
            tabs[i].addEventListener('click', (e) => {
                ...
                let tabContentChildren = e.target.parentElement.nextElementSibling;
								
								
                for (let k = 0; k < tabContentChildren.length; k++) {
									console.log(tabContentChildren[k]) 
